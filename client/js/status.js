    ...
    .factory('Status', function ($rootScope, $ionicPopup, STATUS_LABELS) {
        ...
        function checkStatus(data) {
            if (data.originalStatus !== data.newStatus) {
                ...
                $ionicPopup.show({
                    title: 'Félications, ' + $rootScope.user.firstname + '!',
                    template:
                        '<div style="text-align: center" class="status' + data.newStatus + '">' +
                        '<p>Vous avez atteint un nouveau statut !</p>' +
                        '<img src="css/img/icon-status' + data.newStatus + '.png" height="120"/>' +
                        '<h3 class="status' + data.newStatus + '">Membre ' + STATUS_LABELS[data.newStatus - 1] + ' </h3>' +
